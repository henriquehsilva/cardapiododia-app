...
function Marketplace() {
  ...
  useEffect(() => {
    ...
    if (!firebaseEnabled) {
      let saved = null;
      try {
        saved = JSON.parse(localStorage.getItem("cdd-store") || "null");
      } catch {
    ...
    Promise.race([
      ...
      .catch(async () => {
        try {
          ...
          if (active) setError("Não foi possível carregar os restaurantes agora. Verifique sua conexão e tente novamente.");
        }
      })
      .finally(() => {
    ...
    return () => { active = false; controller.abort(); window.clearTimeout(timeoutId); document.title = "Cardápio do Dia"; };
  }, [loadAttempt]);
  ...
  useEffect(() => {
    ...
    const useCapturedPrompt = () => { setInstallPrompt(window.__cddInstallPrompt || null); setInstallUnavailable(false); if (mobile) setShowInstall(true); };
  ...
  useEffect(() => {
    ...
    return onSnapshot(query(collection(db, "stores"), where("published", "==", true)), async (snapshot) => {
      ...
      snapshot.docChanges().filter((change) => change.type === "added").forEach((change) => {
        ...
        registration?.showNotification("Nova loja no Cardápio do Dia", {
    ...
    <div className="market-page">
      ...
      <main>
        <section className="market-hero">
          <div><p className="eyebrow">COMIDA BOA PERTO DE VOCÊ</p><h1>Descubra novos sabores na sua região.</h1><p>Explore restaurantes e cozinhas locais, escolha seus pratos e peça diretamente de quem prepara.</p></div>
          <form className="market-search" onSubmit={(event) => event.preventDefault()} role="search">
            <label className="market-search-term"><span aria-hidden="true">⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="O que você está procurando?" aria-label="Buscar restaurantes, pratos ou categorias" />{search && <button type="button" onClick={() => setSearch("")} aria-label="Limpar busca">×</button>}</label>
          ...
          <p className="market-location-status">⌖ {locationCity ? `Mostrando restaurantes em ${locationCity} · ${locationState}` : locationState ? `Selecione uma cidade de ${locationState}` : "Mostrando restaurantes de todo o Brasil"}</p>
          <div className="market-suggestions"><span>Mais buscados:</span>{["Marmitas", "Lanches", "Doces", "Bebidas"].map((item) => <button key={item} onClick={() => setSearch(item)}>{item}</button>)}</div>
        </section>
      ...
      {showInstall && (
        <aside className="install-app-card" role="dialog" aria-label="Instalar aplicativo Cardápio do Dia">
          <img src="/cardapiododia-app-logo.png" alt="" />
          <div>
            <strong>Instale o Cardápio do Dia</strong>
