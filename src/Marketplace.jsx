...
import { normalizeStore } from "./storeCompatibility";
...
function Marketplace() {
  ...
  useEffect(() => {
    document.title = "Descubra lojas | Cardápio do Dia";
    setLoading(true);
    setError("");
    if (!firebaseEnabled) {
      let saved = null;
      ...
      setStores([...(saved?.published ? [saved] : []), demoStore, ...demoMarketplaceStores].map(normalizeStore));
      setLoading(false);
    ...
    Promise.race([
      ...
      .then((loadedStores) => active && setStores(loadedStores.map(normalizeStore)))
      .catch(async () => {
        try {
          ...
          if (active) setStores(snapshot.docs.map((item) => normalizeStore({ id: item.id, ...item.data() })));
        } catch {
    ...
    <div className="market-page">
      ...
      <main>
        <section className="market-hero">
          <div><p className="eyebrow">O MARKETPLACE DOS PEQUENOS NEGÓCIOS</p><h1>Encontre algo especial perto de você.</h1><p>Explore lojas independentes, descubra pratos únicos e compre diretamente de quem faz acontecer.</p></div>
          <form className="market-search" onSubmit={(event) => event.preventDefault()} role="search">
            <label className="market-search-term"><span aria-hidden="true">⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="O que você está procurando?" aria-label="Buscar lojas, pratos ou categorias" />{search && <button type="button" onClick={() => setSearch("")} aria-label="Limpar busca">×</button>}</label>
        ...
        <section className="market-cta"><div><p className="eyebrow">VENDA DO SEU JEITO</p><h2>Quer ver sua loja aqui?</h2><p>Crie sua vitrine, publique seus pratos e seja encontrado por novos clientes.</p></div><Link className="button light" to="/admin">Criar minha loja grátis →</Link></section>
      </main>
