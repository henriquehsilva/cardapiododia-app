...
export default function CategoryAutocomplete({ categories, onAdd }) {
  ...
  const suggestions = useMemo(() => categorySuggestions(search, categories.map((item) => typeof item === 'string' ? item : item.name)), [search, categories]);
  const choose = (name) => { onAdd(name); setSearch(""); setOpen(false); };

  return <div className="category-autocomplete">
    <label><span>Buscar categoria</span><div><span aria-hidden="true">⌕</span><input value={search} onFocus={() => setOpen(true)} onChange={(event) => { setSearch(event.target.value); setOpen(true); }} placeholder="Ex.: marmitas, bebidas, sobremesas…" autoComplete="off" /></div></label>
