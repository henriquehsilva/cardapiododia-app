...
export default function AdminFeatures({ store, items, setItems, update, save, notice }) {
  ...
  const downloadReport = async () => {
    ...
    await generatePeriodPdf({ store: { ...store, brand: store.brand || store.navbar?.brand || '' }, orders: reportOrders, start, end, appUrl: location.origin });
  };
  ...
  return <div className="admin-features">
    ...
    <section><p className="eyebrow">PAGAMENTOS</p><h2>Pix e cartão à vista</h2><label className="feature-check"><input type="checkbox" checked={Boolean(store.payment?.enabled)} onChange={event => update('payment.enabled', event.target.checked)} />Aceitar Pix com QR Code</label><input placeholder="Chave Pix" value={store.payment?.pixKey || store.pixKey || ''} onChange={event => update('payment.pixKey', event.target.value)} /><input placeholder="Nome do recebedor" value={store.payment?.pixReceiverName || store.brand || store.navbar?.brand || ''} onChange={event => update('payment.pixReceiverName', event.target.value)} /><input placeholder="Cidade do recebedor" value={store.payment?.pixCity || ''} onChange={event => update('payment.pixCity', event.target.value.toUpperCase())} /><button className="button outline" type="button" disabled={!store.id || busy} onClick={connectStripe}>{store.payment?.stripeConnected ? 'Gerenciar conta Stripe' : 'Conectar Stripe'}</button><small>Cartões e carteiras digitais são sempre cobrados à vista.</small></section>
