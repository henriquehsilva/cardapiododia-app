...
import { normalizeMenuItems, normalizeStore } from "./storeCompatibility";
      ...
      <main>
        <section className="landing-hero">
          <div>
            ...
            <p>
              Publique pratos variados, controle a disponibilidade e receba
...
function StorePage() {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => document.getElementById(`prato-${productId}`)?.scrollIntoView({ block: "center" }));
  }, [productId, products]);
  useEffect(() => {
    if (!firebaseEnabled) {
      ...
      setProducts(normalizeMenuItems(
        saved?.slug === slug
          ? localProducts() || []
          : slug === demoStore.slug || marketplaceStore
            ? demoProducts
            : [],
        saved?.slug === slug ? saved : demoStore,
      ));
      setLoading(false);
      return;
    }
    ...
    getDocs(
      ...
      const loadedStore = normalizeStore({ id: d.id, ...d.data() });
      setStore(loadedStore);
      unsub = onSnapshot(collection(db, "stores", d.id, "menuItems"), (snap) => {
        setProducts(normalizeMenuItems(snap.docs.map((x) => ({ id: x.id, ...x.data() })), loadedStore));
        setLoading(false);
      });
    });
    return () => unsub();
        ...
        >
          <section className="cart">
            ...
            <div className="delivery-payment-option">
              <button className="button delivery-payment-button full" disabled={paying} onClick={payOnDelivery}>{paying ? "Registrando pedido…" : "Pagar na entrega"}<small>Pagamento à vista na entrega ou retirada</small></button>
            </div>
            {error && <p className="error">{error}</p>}
          </section>
        </div>
...
function Admin({ user, onLogout }) {
  ...
  useEffect(() => {
    if (!user) return;
    if (!firebaseEnabled) {
      const loadedStore = normalizeStore(localStore() || emptyStore(user.uid));
      setStore(loadedStore);
      setProducts(normalizeMenuItems(localProducts() || demoProducts, loadedStore));
    ...
    getDocs(query(collection(db, "stores"), where("ownerId", "==", user.uid)))
      .then(async (snap) => {
        if (snap.empty) setStore(emptyStore(user.uid));
          ...
          setProducts(normalizeMenuItems(p.docs.map((x) => ({ id: x.id, ...x.data() })), loadedStore));
        }
      })
      .catch((error) => {
  ...
  const save = async (overrides = {}, advance = true) => {
    ...
    const missingStock = products.find((product) =>
      product.stock !== undefined && product.stock !== null && product.stock !== ""
    ...
    try {
      ...
      if (!overrides.published && advance) {
        const steps = ["store", "categories", "menuItems", "payment", "publish"];
    ...
    <div className="admin">
      ...
      <div className="editor">
        ...
        <main className="editor-form">
          ...
          {tab !== "sales" && !(tab === "menuItems" && productEditor) && (
    ...
    <div className="rich-description-field">
      ...
      <div className="rich-description-editor">
        ...
        <textarea
          ...
          maxLength={2000}
          rows={7}
          placeholder="Descreva ingredientes, acompanhamentos, tamanho da porção e diferenciais do prato…"
...
export default function App() {
  ...
  return (
    <Routes>
      ...
      <Route path="/loja/:slug/prato/:productId" element={<StorePage />} />
