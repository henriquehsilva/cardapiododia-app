...
const localStore = () => readStoredJson("cdd-store");
const localProducts = () => readStoredJson("cdd-products");
const saveLocal = (store, products) => {
  safeStorageSet("cdd-store", JSON.stringify(store));
  safeStorageSet("cdd-products", JSON.stringify(products));
};
    ...
    <div className="landing-page">
      <header className={`marketing-nav ${menuOpen ? "menu-open" : ""}`}>
        ...
        <nav id="marketing-menu">
          <Link to="/lojas" onClick={() => setMenuOpen(false)}>Restaurantes</Link>
      ...
      <main>
        ...
        <section className="business-app-section">
          <div className="business-app-copy">
            ...
            <div className="business-app-points">
              <span><b>01</b> Mais facilidade para fazer pedidos</span>
        ...
        <section className="how">
          ...
          <h2>Da cozinha ao pagamento.</h2>
          <div className="steps">
            <article>
              <b>01</b>
              <h3>Monte a vitrine</h3>
              <p>Cadastre sua marca, pratos, fotos e preços.</p>
            </article>
            <article>
              <b>02</b>
              <h3>Controle a disponibilidade</h3>
              <p>Marque os itens que não estão disponíveis para pedido.</p>
            </article>
            <article>
              <b>03</b>
              <h3>Receba pedidos online</h3>
              <p>
        ...
        <section className="cta">
          <h2>Quer receber mais pedidos?</h2>
          <p>Dê ao seu restaurante um cardápio profissional hoje.</p>
          <Link className="button primary" to="/admin">
            Começar agora
          </Link>
        </section>
      </main>
      <footer>© 2026 Cardápio do Dia · Feito para bons negócios.</footer>
      <MobileSiteNav />
      <a
        className="landing-whatsapp"
        href="https://wa.me/5562985849729?text=Ol%C3%A1%21%20Gostaria%20de%20mais%20informa%C3%A7%C3%B5es%20sobre%20o%20Card%C3%A1pio%20do%20Dia."
        target="_blank"
        rel="noreferrer"
...
function Login({ user }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      } else safeStorageSet("cdd-demo-user", email);
      nav("/admin");
    } catch (err) {
      setError(err.message);
    }
...
function StorePage() {
  ...
  const [installPrompt, setInstallPrompt] = useState(() => window.__cddInstallPrompt || null);
  ...
  useEffect(() => {
    ...
    const useCapturedPrompt = () => {
      setInstallPrompt(window.__cddInstallPrompt || null);
      setInstallUnavailable(false);
      if (mobile) setShowInstall(true);
    };
    const installed = () => { setShowInstall(false); setInstallPrompt(null); };
    window.addEventListener("beforeinstallprompt", capturePrompt);
    window.addEventListener("cddinstallpromptready", useCapturedPrompt);
    window.addEventListener("appinstalled", installed);
    return () => {
      window.removeEventListener("beforeinstallprompt", capturePrompt);
      window.removeEventListener("cddinstallpromptready", useCapturedPrompt);
  ...
  useEffect(() => {
    ...
    document.title = `${store.brand} | Cardápio do Dia`;
  ...
  const requestInstall = async () => {
    const prompt = installPrompt || window.__cddInstallPrompt || await window.__cddWaitForInstallPrompt?.();
    if (!prompt) {
    ...
    <div className="store-page" style={paletteStyle(store.palette)}>
      <header className="store-nav">
        <a className="brand" href="#top">
          <img src={store.logoUrl || "/default-logo.svg"} alt={`Logo da ${store.brand}`} />
        ...
        <label className="store-mobile-search">
          <SearchIcon />
          <input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar pratos" aria-label="Buscar pratos" />
      ...
      <main id="top">
        ...
        >
          <div>
            ...
            <a className="button light" href="#pratos">
              Ver pratos
            </a>
          </div>
        </section>
        <section className="products section" id="pratos">
          ...
          <label className="store-search store-desktop-search">
            ...
            <input
              ...
              placeholder="Buscar por prato, categoria, descrição ou preço"
          ...
          <div
            ...
            aria-label={
              displayed.length > 1 ? "Lista de pratos" : "Prato da loja"
            }
          >
      ...
      <footer className="docs-footer">
        <Logo />
        <span>© 2026 Cardápio do Dia · Feito para bons negócios.</span>
      </footer>
      ...
      {cartOpen && (
        ...
        >
          <section className="cart">
            ...
            <div className="delivery-payment-option">
              <button className="button delivery-payment-button full" disabled={paying} onClick={payOnDelivery}>{paying ? "Registrando pedido…" : "Pagar na entrega"}<small>Dinheiro, Pix ou cartão na entrega/retirada</small></button>
            </div>
            {error && <p className="error">{error}</p>}
          </section>
        </div>
...
const createProductShareImage = async (store, product, imageUrl) => {
  ...
  if (!response.ok || !contentType.startsWith("image/"))
    throw new Error("A foto do prato não pôde ser carregada.");
  ...
  context.fillText(String(store.brand || "Cardápio do Dia").toUpperCase(), 70, 920);
  ...
  context.fillText(`cardapiododia.app/loja/${store.slug}`, 70, 1310);
...
function ProductCard({ store, product, quantity, onChange }) {
  ...
  const like = async () => {
    ...
    let visitorId;
    try {
      visitorId = localStorage.getItem("cdd-visitor-id");
      if (!visitorId) {
        visitorId = crypto.randomUUID();
        localStorage.setItem("cdd-visitor-id", visitorId);
      }
    } catch {
      visitorId = crypto.randomUUID();
    }
  ...
  const share = async () => {
    ...
    const previewUrl = `${location.origin}/loja/${store.slug}/prato/${product.id}?foto=${imageVersion}`;
    const message = [
  ...
  return (
    <>
      ...
      >
        ...
        >
          <small>{product.category || "Prato"}</small>
          <h3>
            <button
              type="button"
          ...
          {productDiscount(product) > 0 && (
            <div className="site-discount-message">
              ...
              <span>
                Finalizando sua pedido no site, você tem{" "}
          ...
          {productUnavailable(product) && <em>Indisponível para pedido</em>}
          <div className="product-social">
            <button
              className={liked ? "liked" : ""}
              onClick={like}
              aria-label="Curtir prato"
            >
              <svg viewBox="0 0 24 24" aria-hidden="true">
...
function Admin({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(
      ...
      (error) =>
        setSaved(`Não foi possível carregar as pedidos: ${error.message}`),
    );
  }, [store?.id, user]);
  useEffect(() => {
  ...
  const generateReport = async () => {
    ...
    if (!reportOrders.length) {
      setSaved("Não há pedidos pagas no período selecionado.");
      return;
    }
  ...
  const confirmManualPayment = async (order) => {
    ...
    try {
      if (firebaseEnabled) {
        ...
        setOrders(updated);
        safeStorageSet("cdd-orders", JSON.stringify(updated));
      }
  ...
  const save = async (overrides = {}, advance = true) => {
    ...
    if (products.length && !categories.length) {
      setSaved("Cadastre uma categoria antes de salvar os pratos.");
      setSaving(false);
      setTab("categories");
      return null;
    }
    ...
    if (uncategorized) {
      setSaved(`Escolha uma categoria para o prato “${uncategorized.name}”.`);
      setSaving(false);
      setTab("products");
      return null;
    }
  ...
  const add = () => {
    if (!categories.length) {
      setSaved("Cadastre a primeira categoria antes de adicionar pratos.");
      setTab("categories");
      return;
    }
    setProductEditor({
  ...
  const finishProduct = () => {
    ...
    if (!product?.name.trim()) {
      setSaved("Informe o nome do prato.");
      return;
    }
    if (!productCategoryId(product, categories)) {
      setSaved("Escolha uma categoria para o prato.");
      return;
    }
    ...
    setSaved(
      productEditor.mode === "create"
        ? "Prato adicionado. Salve as alterações para publicar ✓"
        : "Prato atualizado. Salve as alterações para publicar ✓",
    );
  };
  ...
  const removeCategory = (category) => {
    ...
    if (inUse) {
      setSaved(
        "Mova ou exclua os pratos desta categoria antes de removê-la.",
      );
      return;
    }
    update(
        ...
        <main className="editor-form">
          ...
          {tab === "categories" && (
            <>
              ...
              <p className="intro">
                Crie as categorias antes dos pratos. Cada categoria pode
                  ...
                  {products.length > 0 && (
                    <label className="sales-search product-admin-search">
                      ...
                      <input
                        ...
                        aria-label="Buscar pratos cadastrados"
                      />
                      {productSearch && (
                        <button
                          type="button"
          ...
          {tab === "payment" && (
            <>
              ...
              <p className="intro">
                ...
                o código copia e cola com o valor exato da pedido.
              </p>
                  ...
                  <p>
                    {store.payment?.stripeConnected
                      ...
                      : store.payment?.stripeAccountId ||
                        ...
                        : "Conecte uma conta Stripe para receber diretamente pelas pedidos da sua loja."}
                  </p>
                  {(store.payment?.stripeAccountId ||
                    Object.keys(store.payment?.stripeAccountIds || {}).length >
                      0) && (
            ...
            <>
              ...
              <label className="sales-search">
                ...
                <input
                  ...
                  placeholder="Buscar cliente, prato, contato ou pagamento"
          ...
          {tab === "closing" && (
            <>
              ...
              <p className="admin-help">
                Selecione as datas para baixar um PDF com as pedidos pagas,
                clientes, itens, valores e meios de pagamento.
              </p>
              <section className="closing-card">
                ...
                {!reportOrders.length && reportStart && reportEnd && (
                  <small className="closing-empty">
                    Nenhuma pedido paga encontrada neste período.
                  </small>
                )}
              </section>
            </>
            ...
            <>
              ...
              <div className="publish-box">
                ...
                <div className="publish-store-qr">
                  {storeQrCode ? <img src={storeQrCode} alt={`QR Code do endereço da loja ${store.brand || "Cardápio do Dia"}`} /> : <span>Gerando QR Code…</span>}
        ...
        <div className="preview-screen">
          ...
          <div className="preview-products">
            ...
            {visible.length ? (
              ...
              <p>Adicione pratos para preencher sua vitrine.</p>
            )}
          </div>
          ...
          <footer>
            <b>cardápio do dia</b>
            <small>Feito para negócios com sabor.</small>
          </footer>
        </div>
      </div>
    </aside>
...
function ProductDescription({ value, preview = false }) {
  ...
  if (!content) return preview ? <p>Confira os detalhes deste prato.</p> : null;
  return (
    ...
    <div className="rich-description-field">
      ...
      <div className="rich-description-editor">
        ...
        <textarea
          ...
          maxLength={2000}
          rows={7}
          placeholder="Apresente os benefícios, materiais, medidas e diferenciais do prato…"
...
function ProductImagesUpload({ values, onUpload, onChange }) {
  ...
  return (
    <div className="multi-upload">
      <span>
        Fotos do prato <b>{values.length}/10</b>
      </span>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const email = safeStorageGet("cdd-demo-user");
  ...
  const logout = async () => {
    if (firebaseEnabled) await signOut(auth);
      ...
      safeStorageRemove("cdd-demo-user");
      setUser(null);
    }
  };
  return (
