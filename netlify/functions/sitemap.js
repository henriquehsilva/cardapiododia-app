...
export default async function (request) {
  try {
    ...
    const dynamicUrls = await Promise.all(storesSnap.docs.map(async (storeDoc) => {
      ...
      const products = await storeDoc.ref.collection("menuItems").get();
      const modified = isoDate(store.updatedAt);
      return [
        { loc: `${origin}/loja/${encodeURIComponent(store.slug)}`, lastmod: modified, priority: "0.8" },
        ...products.docs.map((product) => ({ loc: `${origin}/loja/${encodeURIComponent(store.slug)}/prato/${encodeURIComponent(product.id)}`, lastmod: isoDate(product.data().updatedAt || store.updatedAt), priority: "0.7" })),
      ];
    }));
    const urls = [
