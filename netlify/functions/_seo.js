...
export const productSchema = ({ baseUrl, store, product, productId }) => {
  const url = `${baseUrl}/loja/${encodeURIComponent(store.slug)}/prato/${encodeURIComponent(productId)}`;
  const discount = Math.min(99, Math.max(0, Number(product.discountPercent ?? product.cashbackPercent) || 0));
