...
export default async function (request) {
  try {
    ...
    if (!storeRef || !productId)
      return new Response("Prato não encontrado.", { status: 404 });
    const [storeSnap, productSnap] = await Promise.all([
      storeRef.get(),
      storeRef.collection("menuItems").doc(productId).get(),
    ]);
    ...
    const destination = `${baseUrl}/loja/${store.slug}#prato-${productId}`;
    ...
    const publicUrl = `${baseUrl}/loja/${store.slug}/prato/${productId}`;
    ...
    return new Response("Não foi possível abrir o prato.", { status: 500 });
  }
}
