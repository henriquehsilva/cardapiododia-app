...
import { normalizeStoreData } from './_store.js';
...
export default async function (request) {
  try {
    ...
    const store = normalizeStoreData(storeSnap.data());
    const productsSnap = await firestore.collection("stores").doc(storeSnap.id).collection("menuItems").get();
    ...
    const description = plainText(store.tagline || store.description || `Conheça a loja ${store.brand} e descubra seus pratos.`);
    ...
    const storeSchema = {
      ...
      logo: absoluteUrl(baseUrl, store.logoUrl || "/default-logo.svg"),
      ...
      sameAs,
      hasOfferCatalog: { "@type": "OfferCatalog", name: `Pratos de ${store.brand}`, itemListElement: products.map((product, index) => ({ "@type": "ListItem", position: index + 1, item: productSchema({ baseUrl, store, product, productId: product.id }) })) },
    };
    const productLinks = products.map((product) => `<article><h2><a href="${escapeHtml(`${publicUrl}/prato/${product.id}`)}">${escapeHtml(product.name)}</a></h2><p>${escapeHtml(plainText(product.description, 240))}</p><strong>R$ ${escapeHtml(Number(product.price || 0).toFixed(2).replace(".", ","))}</strong></article>`).join("");
    const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><meta name="description" content="${escapeHtml(description)}"><meta name="robots" content="index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1"><meta property="og:type" content="website"><meta property="og:locale" content="pt_BR"><meta property="og:site_name" content="Cardápio do Dia"><meta property="og:title" content="${escapeHtml(title)}"><meta property="og:description" content="${escapeHtml(description)}"><meta property="og:url" content="${escapeHtml(publicUrl)}"><meta property="og:image" content="${escapeHtml(socialImage)}"><meta property="og:image:secure_url" content="${escapeHtml(socialImage)}"><meta property="og:image:type" content="image/jpeg"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:image:alt" content="${escapeHtml(imageAlt)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escapeHtml(title)}"><meta name="twitter:description" content="${escapeHtml(description)}"><meta name="twitter:image" content="${escapeHtml(socialImage)}"><link rel="canonical" href="${escapeHtml(publicUrl)}"><link rel="alternate" hreflang="pt-BR" href="${escapeHtml(publicUrl)}"><script type="application/ld+json">${safeJsonLd(storeSchema)}</script></head><body><header><a href="/">Cardápio do Dia</a></header><main><h1>${escapeHtml(store.brand)}</h1><p>${escapeHtml(description)}</p><p>${escapeHtml(store.address || "Loja online")}</p><section aria-label="Pratos">${productLinks || "<p>Catálogo em atualização.</p>"}</section></main><footer><a href="${escapeHtml(publicUrl)}">Visitar ${escapeHtml(store.brand)}</a></footer></body></html>`;
